...
function setupNavigation(){const mobileMenu=document.getElementById("mobile-menu");const navLinks=document.getElementById("nav-links");const header=document.getElementById("header");if(!mobileMenu||!navLinks||!header)return;mobileMenu.addEventListener("click",function(){toggleMobileMenu(mobileMenu,navLinks)});const navLinkItems=navLinks.querySelectorAll("a");navLinkItems.forEach((link)=>{link.addEventListener("click",function(){closeMobileMenu(mobileMenu,navLinks)})});document.addEventListener("click",function(event){const isClickInsideNav=mobileMenu.contains(event.target)||navLinks.contains(event.target);if(!isClickInsideNav&&navLinks.classList.contains("active")){closeMobileMenu(mobileMenu,navLinks)}});document.addEventListener('touchmove',function(e){if(document.body.classList.contains('menu-open')){if(!navLinks.contains(e.target)){e.preventDefault()}}},{passive:!1});setupSmoothScrolling()}
function toggleMobileMenu(menuButton,navLinks){menuButton.classList.toggle("active");navLinks.classList.toggle("active");const expanded=menuButton.getAttribute("aria-expanded")==="true"||!1;menuButton.setAttribute("aria-expanded",!expanded);if(navLinks.classList.contains("active")){const scrollY=window.scrollY;document.body.style.position='fixed';document.body.style.top=`-${scrollY}px`;document.body.style.width='100%';document.body.classList.add('menu-open')}else{const scrollY=document.body.style.top;document.body.style.position='';document.body.style.top='';document.body.style.width='';document.body.classList.remove('menu-open');if(scrollY){window.scrollTo(0,parseInt(scrollY||'0')*-1)}}}
function closeMobileMenu(menuButton,navLinks){menuButton.classList.remove("active");navLinks.classList.remove("active");menuButton.setAttribute("aria-expanded","false");const scrollY=document.body.style.top;document.body.style.position='';document.body.style.top='';document.body.style.width='';document.body.classList.remove('menu-open');if(scrollY){window.scrollTo(0,parseInt(scrollY||'0')*-1)}}
...
function setupScrollEffects(){const header=document.getElementById("header");const sections=document.querySelectorAll(".section");const navLinks=document.querySelectorAll(".nav-links a");if(!header)return;let ticking=!1;let lastScrollY=0;const scrollHandler=function(){const currentScrollY=window.scrollY;if(Math.abs(currentScrollY-lastScrollY)<3)return;if(!ticking){requestAnimationFrame(function(){handleHeaderScroll(header);handleActiveNavigation(sections,navLinks);handleParallaxEffect();handleScrollProgress();lastScrollY=currentScrollY;ticking=!1});ticking=!0}};if(supportsPassive()){window.addEventListener("scroll",scrollHandler,{passive:!0})}else{window.addEventListener("scroll",scrollHandler)}setupScrollProgressBar();setupRevealAnimations()}
...
function handleParallaxEffect(){const hero=document.querySelector('.hero');const heroBackground=document.querySelector('.hero-background');if(hero&&heroBackground){const scrolled=window.pageYOffset;const rate=scrolled*-0.5;heroBackground.style.transform=`translate3d(0, ${rate}px, 0)`}}
function handleScrollProgress(){const winScroll=document.body.scrollTop||document.documentElement.scrollTop;const height=document.documentElement.scrollHeight-document.documentElement.clientHeight;const scrolled=(winScroll/height)*100;const progressBar=document.getElementById('scroll-progress');if(progressBar){progressBar.style.width=scrolled+'%'}}
function setupScrollProgressBar(){const progressBar=document.createElement('div');progressBar.id='scroll-progress';progressBar.className='scroll-progress-bar';document.body.appendChild(progressBar)}
function setupRevealAnimations(){const revealElements=document.querySelectorAll('.section-title, .about-text p, .contact-item, .skill-category h3, .service-card h3');const revealObserver=new IntersectionObserver((entries)=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add('reveal-animate');revealObserver.unobserve(entry.target)}})},{threshold:0.2,rootMargin:'0px 0px -50px 0px'});revealElements.forEach(el=>{el.classList.add('reveal-element');revealObserver.observe(el)})}
...
img.decoding='async';img.style.opacity='1';img.style.transition='opacity 0.3s ease';img.style.transform='translateZ(0)';img.style.contain='layout';if(!img.style.width&&!img.style.height){img.style.aspectRatio='16/10';img.style.width='100%';img.style.height='auto'}
if(img.closest('.hero')){img.fetchPriority='high';img.loading='eager';if(isMobile&&img.src.includes('images/')){img.style.maxHeight='300px';img.style.objectFit='cover'}}
img.onerror=function(){this.style.backgroundColor='#1a1a2e';this.style.minHeight=isMobile?'150px':'200px';this.style.display='block'};if(img.src){img.dataset.originalSrc=img.src}
if(!img.closest('.hero')&&img.dataset.src){imageObserver.observe(img)}})};function triggerEntranceAnimations(){const heroContent=document.querySelector(".hero-content");if(heroContent){heroContent.classList.add("animate-in")}}
...
submitButton.innerHTML='<i class="fas fa-spinner fa-spin"></i> Sending...';submitButton.disabled=!0;fetch(form.action,{method:"POST",body:formData,headers:{Accept:"application/json",},}).then((response)=>{if(response.ok){showNotification("Message sent successfully! I'll get back to you soon.","success",);form.reset();window.scrollTo({top:0,behavior:"smooth"})}else{throw new Error("Network response was not ok")}}).catch((error)=>{console.error("Error:",error);showNotification("There was a problem sending your message. Please try again later.","error",);window.scrollTo({top:0,behavior:"smooth"})}).finally(()=>{submitButton.innerHTML=originalText;submitButton.disabled=!1})}
...
function showNotification(message,type="info"){const notification=document.createElement("div");notification.className=`notification notification-${type}`;const isIndexForm=window.location.pathname.includes('index.html')||window.location.pathname==='/'||window.location.pathname==='';const indexForm=document.getElementById('index-contact-form');if(isIndexForm&&indexForm){notification.style.position='absolute';notification.style.top='auto';notification.style.bottom='100%';notification.style.right='20px';notification.style.marginBottom='10px';notification.style.zIndex='10000';indexForm.style.position='relative';indexForm.appendChild(notification)}else{document.body.appendChild(notification)}
...
function setupBackToTop(){const backToTopButton=document.getElementById("back-to-top");if(!backToTopButton){console.warn('Back to top button not found');return}backToTopButton.removeAttribute('style');const throttledScrollHandler=throttle(function(){const scrolled=window.scrollY||document.documentElement.scrollTop;if(scrolled>50){backToTopButton.classList.add("visible")}else{backToTopButton.classList.remove("visible")}},16);window.addEventListener("scroll",throttledScrollHandler,passiveIfSupported);backToTopButton.addEventListener("click",function(e){e.preventDefault();window.scrollTo({top:0,behavior:"smooth"})});throttledScrollHandler()}
